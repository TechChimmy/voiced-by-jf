...
import { Play, Download, Pause, Volume2, VolumeX, ChevronLeft, ChevronRight } from 'lucide-react'
...
const SampleNextArrow = (props: any) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <ChevronRight className="text-[#ffde4f] w-6 h-6" />
    </div>
  );
};
...
const SamplePrevArrow = (props: any) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <ChevronLeft className="text-[#ffde4f] w-6 h-6" />
    </div>
  );
};
          ...
          <div className="lg:hidden mb-8">
            <Slider ref={sliderRef} {...settings}>
              {samplesByGenre[activeGenre].map((sample) => (
                <div key={sample.src} className="px-2">
                  <motion.div
                    whileHover={{ y: -5 }}
                    className="rounded-xl bg-[#2a2a2a] overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300 flex flex-col border-t-4 border-[#ffde4f] mx-2"
                  >
